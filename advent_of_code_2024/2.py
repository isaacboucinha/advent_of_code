...
for report in unusual_reports:
    ...
    for i in range(0, len(report) - 1):
        ...
        if not ascending and report[i] < report[i + 1]:
            report_is_safe = False
            break
        levelOfChange = abs(report[i] - report[i + 1])
        if levelOfChange < 1 or levelOfChange > 3:
            report_is_safe = False
            break
    if report_is_safe:
        number_of_safe_reports += 1

print("Number of safe reports: " + str(number_of_safe_reports))

########## Second Part ###########
def change_level_is_safe(first_entry, second_entry):
    change_level = abs(first_entry - second_entry)
    if change_level >= 1 and change_level <= 3:
        return True
    return False

def check_if_unsafe(first_entry, second_entry, third_entry):
    if (first_entry < second_entry and second_entry < third_entry) or (first_entry > second_entry and second_entry > third_entry):
        if change_level_is_safe(first_entry, second_entry) and change_level_is_safe(second_entry, third_entry):
            return False
        else:
            return True
    return True

def get_number_of_safe_reports_without_dampener():
    number_of_safe_reports = 0
    for report in unusual_reports:
        report_is_safe = True
        for i in range(1, len(report) - 1):
            first_entry = report[i - 1]
            second_entry = report[i]
            third_entry = report[i + 1]

            is_unsafe = check_if_unsafe(first_entry, second_entry, third_entry)
            if is_unsafe:
                report_is_safe = False
                break  
...
def get_number_of_safe_reports_with_dampener():
    number_of_safe_reports = 0
    for report in unusual_reports:
        problem_has_been_dampened = False
        previous_value_was_dampened = False
        report_is_safe = True
        for i in range(1, len(report) - 1):
            first_entry = report[i - 1]
            second_entry = report[i]
            third_entry = report[i + 1]
            if previous_value_was_dampened:
                first_entry = report[i - 2]
                if i == 2:
                    # if the first element was wrong, then proceed as usual
                    first_entry = report[i - 1]
                previous_value_was_dampened = False

            is_unsafe = check_if_unsafe(first_entry, second_entry, third_entry)
            if is_unsafe:
                if problem_has_been_dampened:
                    report_is_safe = False
                    break
                else:
                    problem_has_been_dampened = True
                    previous_value_was_dampened = True
...
print('########## Second Part ###########')
print('Number of safe reports without dampener:' + str(get_number_of_safe_reports_without_dampener()))
print("Number of safe reports with dampener: " + str(get_number_of_safe_reports_with_dampener()))
